...
baseline_img = np.fromfile("ig_baseline.bin", dtype=np.float32)
...
iou_list_test_case, dice_list_test_case, iou_case, dice_case = \
    test_cases_late_mask(test_loader_seg=test_cases_loader, device=device, cls_model=VGG19_optimal,
                         threshold=95, fixed_mask=np.ones((224, 224)), baseline_img=baseline_img)
...
iou_list_test_case_msk, dice_list_test_case, iou_case, dice_case = \
    test_cases_late_mask(test_loader_seg=test_cases_loader, device=device, cls_model=VGG19_optimal,
                         threshold=95, fixed_mask=late_mask, baseline_img=baseline_img)
...
iou_list_test_case, dice_list_test_case, iou_case, dice_case = \
    test_cases_late_mask(test_loader_seg=test_cases_loader, device=device, cls_model=Res50_optimal,
                         threshold=95, fixed_mask=np.ones((224, 224)), baseline_img=baseline_img)
...
pd.DataFrame(results_df).to_csv("results_integrated_gradients.csv", index=False, encoding="cp1252")
