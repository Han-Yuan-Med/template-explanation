...
# Now using the Res50
Res50_model = torchvision.models.resnet50(weights=torchvision.models.ResNet50_Weights.DEFAULT)

# Model description
Res50_model.fc = nn.Linear(2048, 2)
print(Res50_model.eval())

for child in Res50_model.children():
    print(child)
...
# Move the input and Res50_model to GPU if it is available
...
# Optimizer SGD
optimizer = optim.SGD(filter(lambda p: p.requires_grad, Res50_model.parameters()), lr=0.001, momentum=0.9)
epoch_num = 50
...
for epoch in range(epoch_num):  # loop over the dataset multiple times
    train_image(cls_model=Res50_model, train_loader=train_loader, criterion=criterion,
    ...
    if auroc_value[epoch] > auroc_value_optimal:
        PATH = f"Res50 optimal.pt"
        torch.save(Res50_model, PATH)
...
results_df = [[f"Res-50", f"{thres_val}",
...
results_df.columns = ['Model', 'Threshold', 'AUROC', 'AUPRC', 'Accuracy', 'Sensitivity', 'Specificity',
                      'PPV', 'NPV']
results_df.to_csv("results_res_50_cls.csv", index=False, encoding="cp1252")
